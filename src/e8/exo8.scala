...
object exo8 extends jacop{
	def main(args: Array[String]): Unit = {
		...
		/*  Parcours options */ val options         = List(28,30,32);
		/*  Exemples 	     */ val etudiants       = ((36,0),(12,24),(15,21),(25,11));
		/*  Exemple testé    */ val cours           = List.fill(etudiants._3._1)(true) ++
				                                          List.fill(etudiants._3._2)(false) zipWithIndex;
		...
		/*  Via la console   */ def afficheur(list:List[_]) = for (cours<-list) println(cours);
		/*  Via le serveur   */
		def serveur(list : List[_]) = {
		...
		def entre45Et120ECTS = {
			...
			/*  UE options       */ val options_vars    = options map(vars(_));
			/*  Parcours options */ def index_options   = List range(0,options_vars length);
			...
			index_options filter(i=> i!=opt ) map (i=>options_vars(i)#=0);
			sum(pondere)>45;
			sum(pondere)<90;
			max(List(10*infraMoins, 7*infraTrop), infraECTS);
			val result = minimize(search(vars,smallest, indomain_max),infractions);
			if(result)for(c <- vars if(c.value()==1))yield c.id();
			else List("No Result !");
		}
		/*  Détermine le cas de l'étudiant et en affiche le nouveau programme de l'étudiant  */
		val pae = if(total<45)moinsDe45ECTS else if(total>120) plusDe120ECTS else entre45Et120ECTS;
		afficheur(pae);
		serveur(pae);
	}
}
