...
object exo6 extends jacop {
	val I1 = 0;val G1 = 1;val I2 = 2;val A1 = 3 ;val I3 = 4; val P1 = 5;val A2 = 6; 
	val I4 = 7;val G2 = 8;val I5 = 9;val D = 10; val A3 = 11;val P2 = 12;                      

	val i1 = BoolVar("Info1");   val g1 = BoolVar("Gestion1");val i2 = BoolVar("Info2");  val a1 = BoolVar("Algo1");
	val i3 = BoolVar("Info3");   val p1 = BoolVar("Projet1"); val a2 = BoolVar("Algo2");  val i4 = BoolVar("Info4");
	val g2 = BoolVar("Gestion2");val i5 = BoolVar("Info5");   val d = BoolVar("Droit");   val a3 = BoolVar("Algo3");
	...
	val infraB1 = sum(block1);
	val infraB1p = IntVar("infraction bloc 1 processé", 1, 30);
	val infraB2 = sum(block2);
	val infraB2p = IntVar("infraction bloc 2 processé", 1, 30);
	...
	val infractions = infractionP + infractionECTS + 30*infraB1p + 30*infraB2p + 40*sum(prerequisVersCorequis.last);
	...
	def resolve(credits: Int, etudiant: List[Boolean]): Unit = {
			...
			for( cours <- tous_les_cours ){
				if(etudiant(cours))vars(cours)#=0;
				for (requis <- corequis_de(cours)if (!etudiant(requis)))vars(cours)->(vars(requis)#=1);
				for(requis <- prerequis(cours)if (!etudiant(requis)))vars(cours)->(vars(requis)#=1);
			};
			sum(pondere)>4;
